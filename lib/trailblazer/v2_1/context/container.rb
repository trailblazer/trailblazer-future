  ...
  module Context
    class Container
      ...
      class UseWithAliases < RuntimeError
        def message
          %{Pass `Trailblazer::V2_1::Context::Container::WithAliases` as `container_class` while defining `aliases`}
        end
      end
      ...
      def inspect
        %{#<Trailblazer::V2_1::Context::Container wrapped_options=#{@wrapped_options} mutable_options=#{@mutable_options}>}
      end
