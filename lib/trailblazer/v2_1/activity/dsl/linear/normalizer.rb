      ...
      module Linear
        ...
        module Normalizer
          ...
          def activity_normalizer(sequence)
            ...
            seq = Trailblazer::V2_1::Activity::Path::DSL.prepend_to_path( # this doesn't particularly put the steps after the Path steps.
              seq,
...
# pp seq
          ...
          def normalize_step_interface((ctx, flow_options), *)
            options = case ctx[:options]
                      when Hash
                        ...
                        if task.is_a?(Symbol)
                          ...
                          { **ctx[:options], id: (id || task), task: Trailblazer::V2_1::Option( task ) }
                        else
            ...
            return Trailblazer::V2_1::Activity::Right, [ctx.merge(options: options), flow_options]
          end

          def wrap_task_with_step_interface((ctx, flow_options), **)
            return Trailblazer::V2_1::Activity::Right, [ctx, flow_options] unless ctx[:wrap_task]
            ...
            return Trailblazer::V2_1::Activity::Right, [ctx.merge(task: wrapped_task), flow_options]
          end

          def normalize_id((ctx, flow_options), **)
            id = ctx[:id] || ctx[:task]

            return Trailblazer::V2_1::Activity::Right, [ctx.merge(id: id), flow_options]
          end
          ...
          def normalize_connections_from_dsl((ctx, flow_options), *)
            ...
            return Trailblazer::V2_1::Activity::Right, [new_ctx, flow_options]
          end
          ...
          def input_output_dsl((ctx, flow_options), *)
            ...
            return Trailblazer::V2_1::Activity::Right, [ctx, flow_options] if config.size == 0 # no :input/:output passed.

            new_ctx = {}
            new_ctx[:extensions] = ctx[:extensions] || [] # merge DSL extensions with I/O.
            new_ctx[:extensions] += [Linear.VariableMapping(**config)]

            return Trailblazer::V2_1::Activity::Right, [ctx.merge(new_ctx), flow_options]
          end
          ...
          def inherit_option((ctx, flow_options), *)
            return Trailblazer::V2_1::Activity::Right, [ctx, flow_options] unless ctx[:inherit]
